...
async def register(interaction: discord.Interaction):
    ...
    if user_ in users:
        ...
        if can_register_now(interaction.user):
            ...
            await interaction.response.send_message(f'Registration is currently only open for returning drivers. An announcement will be posted soon '
...
def can_register_now(user: discord.User) -> bool:
    if const.REG_OPEN_FOR == 'all' or 'season 9' in [role.name for role in user.roles]:
        return True
    else:
        return False
